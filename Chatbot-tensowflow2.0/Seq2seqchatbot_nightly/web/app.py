...
#将语句使用结巴分词进行分词
    ...
    #调用闲聊模式execute的preditc生成回答信息
    res_msg = execute.predict(req_msg)
    #调用问答模式query_answer的search_answer生成问答信息
    rqa_msg_list,socres=query_answer.search_answer(qa_msg)
    #进行一次判断，如果qa的余弦距离小于0.02说明对话是qa可以回答，否则说明qa回答不了，则使用闲聊模式
    if int(socres[0])<0.02:
        res_msg=rqa_msg_list[0]
    #将unk值的词用微笑符号取代
